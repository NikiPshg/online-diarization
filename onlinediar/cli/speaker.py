...
class Speaker:
    ...
    def extract_embedding_feats(self, fbanks, batch_size, subseg_cmn):
        ...
        for i in tqdm(range(0, fbanks_array.shape[0], batch_size)):
            ...
            with torch.no_grad():
                ...
                batch_embs = (
                    batch_embs[-1] if isinstance(batch_embs, tuple) else batch_embs
                )
    ...
    def find_speaker(
        self, embed: torch.Tensor, speaker_dict: dict
    ) -> Tuple[str, float]:
        max_sim = 0
        best_speaker = None

        for speaker, embeddings in speaker_dict.items():
    ...
    def recognize(self, audio_path: str):
        ...
        best_name = None
        best_name, best_score = self.find_speaker(q, self.table)
