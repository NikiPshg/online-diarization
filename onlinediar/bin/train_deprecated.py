...
def train(config="conf/config.yaml", **kwargs):
    ...
    train_utt2spkid_dict = {
        utt_spk[0]: spk2id_dict[utt_spk[1]] for utt_spk in train_utt_spk_list
    }
    if rank == 0:
        logger.info("<== Labels ==>")
        logger.info(
            "train label num: {}, spk num: {}".format(
    ...
    train_dataset = FeatList_LableDict_Dataset(
        train_data_list,
        train_utt2spkid_dict,
    ...
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, **configs["dataloader_args"]
    )
    if rank == 0:
    ...
    if checkpoint is not None:
        ...
        start_epoch = int(re.findall(r"(?<=model_)\d*(?=.pt)", checkpoint)[0]) + 1
    ...
    if rank == 0:
        os.symlink(
            "model_{}.pt".format(configs["num_epochs"]),
