...
def gather_calibration_factors(
    wav_dur_scp, max_dur, score_norm_file, calibration_factor_file, drop_duration=False
):
    if not drop_duration:
        wav_idx, dur_list = zip(*read_table(wav_dur_scp))
        wavidx2dur = {
            idx: min(float(dur), max_dur) for idx, dur in zip(wav_idx, dur_list)
        }
    ...
    def reorder_values(value_1, value_2):
        ...
        return "{:.4f} {:.4f} {:.4f} {:.4f}".format(
            min_value, max_value, max_value - min_value, max_value / min_value
        )

    # read factor from asnorm results
    assert os.path.exists(
        score_norm_file
    ), "score norm file ({}) does not exist !!!".format(score_norm_file)
...
def cllr(target_llrs, nontarget_llrs):
    ...
    return (
        0.5
        * (
            torch.mean(negative_log_sigmoid(target_llrs))
            + torch.mean(negative_log_sigmoid(-nontarget_llrs))
        )
        / np.log(2)
    )
