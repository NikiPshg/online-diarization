...
def get_args():
    ...
    parser.add_argument(
        "--num_frames", default=-1, type=int, help="fix number of frames"
    )
...
def test_mnn_inference(in0, model_path):
    ...
    net = MNN.nn.load_module_from_file(
        model_path, ["feats"], ["embs"], runtime_manager=rt
    )
...
def main():
    ...
    if args.num_frames > 0:
        os.system(
            ...
            --saveStaticModel".format(
                args.onnx_model, args.output_model
            )
        )
    else:
        os.system(
    ...
    # 2. print model info
    os.system("MNNConvert -f MNN --modelFile {} --info".format(args.output_model))
    # 3. check precision
    torch.manual_seed(0)
    if args.num_frames > 0:
    ...
    if np.allclose(onnx_out, mnn_out, rtol=1e-05, atol=1e-02):
        print("Export mnn model successfully, " "and the output accuracy check passed!")
    else:
        print(
            "Export mnn model successfully, but onnx and mnn have different"
