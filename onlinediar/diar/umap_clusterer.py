...
class PAHC:
    ...
    def build_label_map(self):
        ...
        if -1 in self.label_map:
            self.num_labeled -= 1
            for i, j in zip(
                range(self.num_labeled, self.num_labeled + len(self.label_map[-1])),
                self.label_map[-1],
            ):
    ...
    def merge(self, i, j):
        ...
        for k, _ in self.label_map.items():
            ...
            if normalized_cost >= self.merge_cutoff:
                heapq.heappush(self.heap, (-normalized_cost, (k, self.next_index)))
...
def cluster(embeddings, n_neighbors=16, min_dist=0.05):
    ...
    labels = PAHC(merge_cutoff=0.3, min_cluster_size=3, absorb_cutoff=0.0).fit_predict(
        labels, embeddings
    )
    return labels
...
if __name__ == "__main__":
    ...
    run_cluster = functools.partial(cluster, n_neighbors=n_neighbors, min_dist=min_dist)
