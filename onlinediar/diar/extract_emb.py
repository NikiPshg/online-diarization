...
def extract_embeddings(fbanks, batch_size, session, subseg_cmn):
    ...
    for i in tqdm(range(0, fbanks_array.shape[0], batch_size)):
        ...
        batch_embs = session.run(
            input_feed={"feats": batch_feats}, output_names=["embs"]
        )[0].squeeze()
...
def get_args():
    ...
    parser.add_argument(
        "--device", default="cuda", help="inference device type: cpu or cuda"
    )
    parser.add_argument(
        "--batch-size", type=int, default=96, help="batch size for embedding extraction"
    )
    parser.add_argument(
        "--frame-shift",
        type=int,
...
def main():
    ...
    for seg_id, fbank in fbank_dict.items():
        tmp_subsegs, tmp_subseg_fbanks = subsegment(
            fbank, seg_id, window_fs, period_fs, args.frame_shift
        )
        subsegs.extend(tmp_subsegs)
        subseg_fbanks.extend(tmp_subseg_fbanks)
    embeddings = extract_embeddings(
        subseg_fbanks, args.batch_size, session, args.subseg_cmn
    )
