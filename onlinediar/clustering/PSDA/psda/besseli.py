...
class LogBesselI:
    ...
    def __call__(self, k=None, logk=None, exp_scale=False):
        ...
        with np.errstate(divide="ignore"):  # y may be 0 if x ==0
            y[ok] = np.log(y[ok])
        if not exp_scale:
            y[ok] += k[ok]  # undo scaling done by ive
        ...
        if not exp_scale:
            y[nan] += knan  # undo scaling done by ive
...
def fastLogCvmf_e(logI: LogBesselI, d=np.pi, tune=True, quiet=True, err=None):
    """
    This works:
    ...
    if not quiet:
        print(f"  new d={d}, local error = {obj(d)}")
    b, c = bc(d)
    ...
    logk = minimize_scalar(neg_err, (2.0, 4.0)).x
    if not quiet:
        print(f"  new max abs error of {np.sqrt(-neg_err(logk))} at {logk}")
