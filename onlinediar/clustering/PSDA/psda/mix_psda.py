...
class MixPSDA:
    ...
    def __init__(self, p_i: float, within_concentration: float, between_distr: VMF):
        """
    ...
    def zposterior(self, means: ndarray, counts: ndarray):
        ...
        r_tilde = (
            r_tilde[:, None] + np.atleast_1d(self.logCw)[:, None] * counts
        )  # m x s
...
class Side:
    ...
    def __init__(self, psda: MixPSDA, X: ndarray, counts: ndarray):
        ...
        self.yi1norm2 = (
            np.sum(X**2, axis=1, keepdims=True) * psda.w**2
            + X @ (2 * psda.bmu.T * psda.w)
            + psda.b**2
        )
    ...
    def llr_matrix(self, rhs):
        ...
        yi3norm2 = (
            self.yi1norm2[:, None, :]
            + rhs.yi1norm2[None, :, :]
            - self.psda.b[None, None, :] ** 2
        )
        yi3norm2 += (
            2
            * self.psda.w[None, None, :]
            * np.sum(self.X[:, None, :] * rhs.X[None, :, :], axis=-1, keepdims=True)
        )

        logr3 = (self.counts[:, None] + rhs.counts)[:, :, None] * self.psda.logCw[
            None, None, :
        ]
