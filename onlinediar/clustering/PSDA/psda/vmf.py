...
from .besseli import LogBesselI, fast_logrho, fastLogCvmf_e, k_and_logk
from .vmf_sampler import rotate_to_mu, sample_uniform, sample_vmf_canonical_mu
...
class LogNormConst:
    ...
    def __call__(self, k=None, logk=None, fast=False, exp_scale=False):
        ...
        logCe = self.fastlogCvmf_e(k, logk) if fast else self.logCvmf_e(k, logk)
        if exp_scale:
            return logCe
        return logCe - k
    ...
    def rho(self, k=None, logk=None, fast=False):
        ...
        """
        log_rho = self.fastlogrho(k, logk) if fast else self.logrho(k, logk)
...
def compose(norm, mu):
    ...
    if not np.isscalar(norm):
        norm = norm.reshape(*(*mu.shape[:-1], 1))
    return norm * mu
